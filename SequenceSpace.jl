"""
There are continuing issues with the AssempleInpulses function. Type instability occurs when one of the blocks is a heterogeneous agent block. This block is not sparse as the simple blocks are and are a different obe - HetBlock. So when one of these blocks is not the same as other blocks, some type instability occurs when using elements of this HetBlock. 
"""
